...
router.get("/mobilePayments",async (req, res)=>{
    let listData=await servicesModel.find();
    listData=listData[1];
    // console.log( listData)
    res.render('mobilePayments', {data:  listData})
});

 
router.get("/mobilePayments/:id", async(req, res)=>{
    //-------------for displaying list------------
    let listData= await servicesModel.find();
    listData=listData[1];
    ...
    res.render(`mobilePayments/${req.params.id}`,{data:  listData, list_link:{link:req.params.id,service:"mobilePayments"},feedbackData:feedbackData  })
});
...
router.post("/mobilePayments/:id", async (req, res) => {
  if ( req.body.textarea == "" && req.body.name == "" && req.body.email == "") {
    ...
    res.status(201).redirect(`/mobilePayments/${req.params.id}#feedback-redirect`);
