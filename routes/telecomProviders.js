...
router.get("/telecomProviders",async (req, res)=>{
    let listData=await servicesModel.find();
    listData=listData[3];
    // console.log( listData)
    res.render('telecomProviders', {data:  listData})
});

 
router.get("/telecomProviders/:id", async(req, res)=>{
    //-------------for displaying list------------
    let listData= await servicesModel.find();
    listData=listData[1];
    ...
    res.render(`telecomProviders/${req.params.id}`,{data:  listData, list_link:{link:req.params.id,service:"telecomProviders"},feedbackData:feedbackData  })
});
...
router.post("/telecomProviders/:id", async (req, res) => {
  if ( req.body.textarea == "" && req.body.name == "" && req.body.email == "") {
    ...
    res.status(201).redirect(`/telecomProviders/${req.params.id}#feedback-redirect`);
