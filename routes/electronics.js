...
router.get("/electronics",async (req, res)=>{
    ...
    res.render('electronics', {data:  listData})
});
...
router.get("/electronics/:id", async(req, res)=>{
    ...
    res.render(`electronics/${req.params.id}`,{data:  listData, list_link:{link:req.params.id,service:"electronics"},feedbackData:feedbackData  })
});
...
router.post("/electronics/:id", async (req, res) => {
  if ( req.body.textarea == "" && req.body.name == "" && req.body.email == "") {
    ...
    res.status(201).redirect(`/electronics/${req.params.id}#feedback-redirect`);
