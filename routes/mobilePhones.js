...
router.get("/mobilePhones",async (req, res)=>{
    let listData=await servicesModel.find();
    listData=listData[4];
    // console.log( listData)
    res.render('mobilePhones', {data:  listData})
});

 
router.get("/mobilePhones/:id", async(req, res)=>{
    //-------------for displaying list------------
    let listData= await servicesModel.find();
    listData=listData[1];
    ...
    res.render(`mobilePhones/${req.params.id}`,{data:  listData, list_link:{link:req.params.id,service:"mobilePhones"},feedbackData:feedbackData  })
});
...
router.post("/mobilePhones/:id", async (req, res) => {
  if ( req.body.textarea == "" && req.body.name == "" && req.body.email == "") {
    ...
    res.status(201).redirect(`/mobilePhones/${req.params.id}#feedback-redirect`);
