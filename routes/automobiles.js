...
router.get("/automobiles",async (req, res)=>{
    ...
    res.render('automobiles', {data:  listData})
});

 
router.get("/automobiles/:id", async(req, res)=>{
    //-------------for displaying list------------
    let listData= await servicesModel.find();
    listData=listData[1];
    ...
    res.render(`automobiles/${req.params.id}`,{data:  listData, list_link:{link:req.params.id,service:"automobiles"},feedbackData:feedbackData  })
});
...
router.post("/automobiles/:id", async (req, res) => {
  if ( req.body.textarea == "" && req.body.name == "" && req.body.email == "") {
    ...
    res.status(201).redirect(`/automobiles/${req.params.id}#feedback-redirect`);
