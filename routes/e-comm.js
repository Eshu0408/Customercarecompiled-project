...
router.get("/e-comm",async (req, res)=>{
    let listData=await servicesModel.find();
    listData=listData[7];
    // console.log( listData)
    res.render('e-comm', {data:  listData})
});

 
router.get("/e-comm/:id", async(req, res)=>{
    //-------------for displaying list------------
    let listData= await servicesModel.find();
    listData=listData[1];
    ...
    res.render(`e-comm/${req.params.id}`,{data:  listData, list_link:{link:req.params.id,service:"e-comm"},feedbackData:feedbackData  })
});
...
router.post("/e-comm/:id", async (req, res) => {
  if ( req.body.textarea == "" && req.body.name == "" && req.body.email == "") {
    ...
    res.status(201).redirect(`/e-comm/${req.params.id}#feedback-redirect`);
