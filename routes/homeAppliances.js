...
router.get("/homeAppliances",async (req, res)=>{
    ...
    res.render('homeAppliances', {data:  listData})
});

 
router.get("/homeAppliances/:id", async(req, res)=>{
    //-------------for displaying list------------
    let listData= await servicesModel.find();
    listData=listData[1];
    ...
    res.render(`homeAppliances/${req.params.id}`,{data:  listData, list_link:{link:req.params.id,service:"homeAppliances"},feedbackData:feedbackData  })
});
...
router.post("/homeAppliances/:id", async (req, res) => {
  if ( req.body.textarea == "" && req.body.name == "" && req.body.email == "") {
    ...
    res.status(201).redirect(`/homeAppliances/${req.params.id}#feedback-redirect`);
